//
//  SBAIPracticeExpressionRadarView.m
//  SBAIDemo
//
//  Created by 刘永吉 on 2023/6/11.
//

#import "SBAIPracticeExpressionRadarView.h"
#import "SBAIPractice.h"

@interface SBAIPracticeExpressionRadarView ()
...
@implementation SBAIPracticeExpressionRadarView
...
-(void)setModel:(SBAIPracticeAnalyzeModel *)model{
    ...
    [self.valueArray addObject:[NSString stringWithFormat:@"%d",model.scoreServiceability]];
    [self.valueArray addObject:[NSString stringWithFormat:@"%d",model.scoreQuality]];
    [self.valueArray addObject:[NSString stringWithFormat:@"%d",model.scoreAppeal]];
...
//        [path setLineDash:dashPattern count:1 phase:1];//虚线
    ...
    for (int i = 0; i<self.valueArray.count; i++) {
        ...
        CGPoint point = [self calcCircleCoordinateWithCenter:CGPointMake(radius,radius) andWithAngle:i*(360/self.valueArray.count) + 90 andWithRadius:radius];
...
//        [path setLineDash:dashPattern count:1 phase:1];//虚线
    ...
    for (int index = 0; index < self.valueArray.count; index++) {
        ...
        CGPoint point = [self calcCircleCoordinateWithCenter:CGPointMake(radius, radius) andWithAngle:index*(360/self.valueArray.count)+90 andWithRadius:a];
        NSValue *value = [NSValue valueWithCGPoint:point];
        [pointArray addObject:value];
        
        //item point
        CGPoint itemPoint = [self calcCircleCoordinateWithCenter:CGPointMake(radius, radius) andWithAngle:index*(360/self.valueArray.count) + 90 andWithRadius:radius];
    ...
    for (int index = 0; index < itemPointArray.count; index++) {
        ...
        CGFloat angle = index*(360/self.valueArray.count) + 90;//角度
...
-(NSMutableArray *)titleArray{
    if (!_titleArray){
        _titleArray = [NSMutableArray arrayWithArray:@[@"服务能力",@"素质心态",@"感染力"]];
    }
    return _titleArray;
}
